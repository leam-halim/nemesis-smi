...
function seedDatabase(db) {
  ...
  db.transaction(() => {
    ...
    forEachAuditRow(auditSource, (row) => {
      // 1. FILTER KHUSUS KOTA SUKABUMI
      if (process.env.FILTER_BY_REGION === 'true') {
        const lokasiTeks = row.lokasi || "";
        const namaLembaga = row.lembaga || "";

        // Pengecekan ganda: lokasi harus mengandung "(Kota)" 
        // ATAU lembaganya adalah "Kota Sukabumi"
        const isKotaSukabumi = lokasiTeks.includes("Sukabumi (Kota)") || namaLembaga === "Kota Sukabumi";

        if (!isKotaSukabumi) {
          return; // Lewati data Kabupaten atau wilayah lain
        }
      }

      // 2. LANJUT PROSES DATA (Hanya untuk Kota Sukabumi)
